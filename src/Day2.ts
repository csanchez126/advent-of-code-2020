const day2 = (input: string[]) => {
  let validCount = 0;
  input.forEach((str) => {
    const [range, char, pw] = str.split(' ');
    const [min, max] = range.split('-').map((s) => parseInt(s));
    const occurences = pw.split('').filter((c) => c === char[0]).length;
    //Valid pw
...
const day2_2 = (input: string[]) => {
  let validCount = 0;
  input.forEach((str) => {
    const [pos, char, pw] = str.split(' ');
    const [firstPos, secondPos] = pos.split('-').map((s) => parseInt(s));

    if (secondPos <= pw.length) {
      const firstCheck = pw[firstPos - 1] === char[0];
      const secondCheck = pw[secondPos - 1] === char[0];

      validCount += firstCheck != secondCheck ? 1 : 0;
    }
  });
